...
class KoGPT2Chat(LightningModule):
    ...
    def chat(self, sent='0'):
        ...
        self.sent_tokens = self.tok(sent)
        with torch.no_grad():
            print("소켓 서버 시작")
            SocketModule.openSocketServer(self)
    ...
    def getReply(self,q):
        ...
        a_tok = []
        while 1:
            input_ids = torch.LongTensor([self.vocab[U_TKN]] + self.vocab[q_tok] +
                                         self.vocab[EOS, SENT] + self.vocab[self.sent_tokens] +
